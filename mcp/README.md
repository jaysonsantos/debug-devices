...
| `phone_af_mode` | `mode` (`continuous` or `macro`) | Set the autofocus mode. `macro` is the camera's close-range autofocus mode, for work near the minimum focus distance (about 10-12 cm). The result is the camera status with `af_mode`; a phone without the macro mode stays `continuous`, and the result says so. The choice persists (`ui-settings.json`, `af_mode`) and comes back after `phone_connect`, `bench_start`, and an app restart. An app without `af_mode` gives the error "update the phone app". |
...
| `phone_point_to` | `refdes` (one part or a list, up to 8), `registration_id` (default: the newest) | Point the user to parts through the photo registration: a green box when a part is in the phone view, else an arrow at the view edge toward it with the board distance ("J4 ~4 cm"). A part on the other board side gets no arrow, only the note to isolate the power before the user turns the board. With live tracking, the boxes and arrows follow the board while the phone moves. The agent gives the arrow and the distance, never left or right words. |
...
- **Live tracking**: after `board_register_photo`, the server matches the registered snapshot with the current phone screen frame (ORB features and MAGSAC homography, OpenCV). Then it follows the board in the screen frames (about 4 per second, 720 px wide): each frame is matched with the reference frame, or with the last good frame when the view moved far. The map board mm → snapshot pixels is then the registration composed with the tracked camera motion. While the tracking follows, the registration stays valid when the phone moves (the scene watcher does not make it stale), and the boxes and arrows of `phone_point_to` move with the board. The server sends them again only when a box edge moves by 1 % of the image or an arrow turns by 8°, and at most every 0.3 s. After 3 frames without a trusted match, the tracking is lost: the registration becomes stale and the overlay is cleared. The `board_register_photo` result says in `tracking` if the tracking is on, or why not (for example, no phone screen stream). The Board panel shows the tracking state.
- **Arrows**: the page draws the arrows of `phone_point_to` (also a Board panel search) at the edge of the snapshot, in the shown orientation, with the label ("U7301 ~4 cm"). A Board panel search now uses `phone_point_to`: a part outside the view gets an arrow, and a part on the other side gets the note "on the other side: isolate the power before you turn the board".
...
- **Macro focus**: the toggle "Macro focus" in the phone panel and "Macro" in both full-screen bars run `phone_af_mode` (one choice for the three toggles). The panel shows the focus mode of the phone (continuous autofocus, macro, or "not in this app"). When the phone is closer than 15 cm and the mode is continuous, the panel suggests "close range: try Macro focus".
